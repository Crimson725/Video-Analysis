...
def _resolve_face_identity_model_id(enable_face_identity_pipeline: bool) -> str:
    raw_face_identity_model_id = os.getenv("FACE_IDENTITY_MODEL_ID")
    if enable_face_identity_pipeline:
        return normalize_face_identity_model_id(raw_face_identity_model_id)
...
def _storage_settings(default_temp: Path) -> dict[str, Any]:
    return {
        "r2_account_id": _read_str("R2_ACCOUNT_ID"),
        "r2_bucket": _read_str("R2_BUCKET"),
        "r2_access_key_id": _read_str("R2_ACCESS_KEY_ID"),
...
def _corpus_settings() -> dict[str, Any]:
    return {
        ...
        "graph_backend": _read_nonempty_str("GRAPH_BACKEND", "neo4j"),
        "vector_backend": _read_nonempty_str("VECTOR_BACKEND", "pgvector"),
        "neo4j_uri": _read_str("NEO4J_URI", "bolt://127.0.0.1:7687"),
        "neo4j_username": _read_str("NEO4J_USERNAME", "neo4j"),
        "neo4j_password": _read_str("NEO4J_PASSWORD", "local-dev-password"),
        "neo4j_database": _read_str("NEO4J_DATABASE", "neo4j"),
        "pgvector_dsn": _read_str(
            "PGVECTOR_DSN",
            DEFAULT_PGVECTOR_DSN,
        ),
        "embedding_model_id": _read_str("EMBEDDING_MODEL_ID", "gemini-embedding-001"),
        "embedding_model_version": _read_str("EMBEDDING_MODEL_VERSION", "v1"),
        "embedding_dimension": _read_int("EMBEDDING_DIMENSION", default=16, minimum=1),
    }


def _face_identity_settings(
    *,
