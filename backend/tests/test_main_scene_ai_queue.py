"""Tests for queue-mode orchestration in app.main.process_video."""

from types import SimpleNamespace
from unittest.mock import MagicMock, patch

from app import jobs
from app.scene_task_queue import InMemorySceneTaskQueue
...
def test_process_video_enqueues_scene_task_in_queue_mode():
    ...
    assert "scene_task_id" in job
    task = queue.get_task(task_id=job["scene_task_id"])
...
def test_process_video_disabled_scene_pipeline_keeps_stable_empty_outputs():
    queue = InMemorySceneTaskQueue()
    job_id = jobs.create_job()
